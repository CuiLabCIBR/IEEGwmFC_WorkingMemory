...
for nSub = 1:length(subjG)
    ...
    for nTask = 1:length(taskG)
        ...
        gmTFRCC.TFRsignals = TFRsignalsCC;
        gmTFRCC.dim = {'freq', 'time', 'trial', 'channel'};
        gmTFRCC.freq = foi;
        gmTFRCC.time = [0, 2];
        gmTFRCC.fsample = fsample;
        gmTFRCC.chanLabel = chanlabel;
        gmTFRCC.task = taskName;
        savename = [subID, '_task-', taskName, '_CCTFRsignals.mat'];
        savefolder = fullfile('s7_cleanCorrectTFRsignals', subID);
        mkdir(savefolder);
        save(fullfile(savefolder, savename), 'gmTFRCC', '-v7.3');
    end
end