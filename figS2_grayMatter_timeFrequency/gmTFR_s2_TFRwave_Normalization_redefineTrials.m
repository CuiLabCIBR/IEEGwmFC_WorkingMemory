%% Normalization of time frequency representation and redefine trials using task triger
clc; clear; close all;
addpath Z:\xwiEEG_WorkingMemory_zhangshen\github_replication\function\iEEGPrep
iEEGPrep_initial;
datafolder = 's1_channelTFR';
...
for nSub = 1:length(subG)
    ...
    for nChan = 1:chanLength
        ...
        for nTask = 1:length(taskG)
            ...
            dataDir = dir(fullfile(datafolder, subID, [subID, '_task-', taskName, '_chan-', num2str(nChan, '%03d'), '_channelTFR.mat']));
    ...
    % redefine trials
        for nTask = 1:length(TFRwave)
            ...
            switch taskName
                case 'resting'
                    ...
                    savefolder = fullfile('s2_channelTFR_redefineTrials', subID);
                    mkdir(savefolder);
                ...
                case {'0back', '1back', '2back'}
                    ...
                    ieegPrepFolder = 's1_data_afterPrep';
                    TRIGdir = dir(fullfile(ieegPrepFolder, [subID, '_task-', taskName, '_run-1_ref-*_preproc_ieeg.mat']));
        ...
        disp(['complete ', subID, ' channel-', coi]);
    end
end