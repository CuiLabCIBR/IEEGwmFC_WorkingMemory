%% Calculate mena TFR across trials and Normlaization to resting baseline
clc; clear; close all;
            ...
            for nTrial = 1:length(correctIndex)
                if correctIndex(nTrial) == 1
                    ...
                    if sum(isnan(A(:)))==0
                        trialN = trialN + 1;
                        TFRmean(:, :, trialN) = A;
                        trialList(trialN) = nTrial; 
                    end
                end
            end
            TrialList{nTask} = trialList;
...
%% find correct channel atlas
        ...
        for nChan2 = 1:size(gmGoodChans, 1)
           if  strcmpi(gmGoodChans{nChan2, 1}, coi)
               channelLabel = coi;
               disp(channelLabel);
               channelAtlas1 = gmGoodChans{nChan2, 2};
               disp(channelAtlas1);
               channelAtlas2 = gmGoodChans{nChan2, 6};
               disp(channelAtlas2);
               channelAtlas3 = gmGoodChans{nChan2, 7};
               disp(channelAtlas3);
               AtlasP = gmGoodChans{nChan2, 3};
               MNI = gmGoodChans{nChan2, 4};
           end
        end
...
%% plot
        ...
        title([subjID, ' task ', channelLabel, ' ', channelAtlas1, ' ', channelAtlas2, ' ', channelAtlas3]);
        hold off;
        pause(1);
        saveFolder = 's3_TFRmeanAcrossAllRightTrials';
        mkdir(saveFolder);
        mkdir(fullfile(saveFolder, subjID));
        savefig(fullfile(saveFolder, subjID, [channelAtlas1, '_', channelAtlas2, '_', channelAtlas3, '_chan-', num2str(nChan, '%03d'), '.fig']));
        close all;
    end
